...
router.post("/", verifyToken, async (req, res) => {
    try {
       ...
       const updateCategoryNumber = await Categories.findByIdAndUpdate(req.body.categoriesId, {
        $set: {
...
router.put("/", verifyToken, async (req, res) => {
  try {
  const categoryProduct = await CategoryProduct.findById(req.body.categoryProductId);
  if (req.body.categoryProductId === String(categoryProduct._id)) {
      ...
      const categoryProductUpdate = await CategoryProduct.findById(req.body.categoryProductId);
...
router.delete("/:id", verifyToken, async (req, res) => {
    ...
    if (req.params.id === String(categoryProduct._id)) {
      try {
        const categories = await Categories.findById(categoryProduct.categoriesId);

        const updateCategoryNumber = await Categories.findByIdAndUpdate(categoryProduct.categoriesId, {
         $set: {
           categoryProductNumber: categories.categoryProductNumber - 1
          }
       });
...
router.post("/filter", verifyToken, async (req, res) => {
    ...
    try {
        const categoryProduct = await CategoryProduct.find({ title: new RegExp(titleFilter, 'i') }).exec();
      return res.status(200).json({
        success: true,
